...
const compile = (input, helpers) => {
	...
	for (let i = 0; i < cycle_colours.length; ++i) {
		cycle_colours[i] = cycle_colours[i].replaceAll(/[^a-fA-F0-9]/g, "").padStart(6, 0).substring(0,6);
		let r;
		let g; 
		let b;
	...
	for (let i = 0; i < cycle_palettes_start.length; ++i) {
		...
		cycle_end_hex[i] = OKLab_to_hex(OK_L_gradient.at(-1), OK_a_gradient.at(-1), OK_b_gradient.at(-1));
		cycle_start_hex[i]   = OKLab_to_hex(OK_L_gradient.at(0), OK_a_gradient.at(0), OK_b_gradient.at(0));
	}
