...
const id = "GF_EVENT_FADE_STREET_SPECIAL_CYCLE";
const groups = ["Fade Street"];
const subGroups = {"Fade Street": "Colour Mode - Fade And Colour Cycle"};
const description = "Like a Special Fade event with the option of cycling colours through selected palette slots. The base fade is between two selected palettes, with an optional preset effect applied to each. The cycle loops seamlessly. If the total length of the fade (in time) is longer than the length of a cycle, extra iterations of the cycle will be inserted so that the event ends exactly as a cycle ends."; 
const name = "Special Fade with Colour Cycle";
...
const autoLabel = (fetchArg) => {
	...
	return `Special Fade with Colour Cycle (${total_frames} frames, ${cycle_count} full cycles)`;
}
...
const fields = [
	{
		...
		fields: [
			{
				label: "Fade from:",
				description: "Preset effect to apply to the start point of the fade gradient.",
				width: "50%",
			},
			{
				defaultValue: "normal",
	...
	},
	{
		...
		fields: [
			{
				label: "Fade to:",
				description: "Preset effect to apply to the end point of the fade gradient.",
				width: "50%",
			},
			{
				defaultValue: "normal",
...
const compile = (input, helpers) => {
	...
	gbvm += `;     ~~~ End of Special Fade with Colour Cycle block ~~~\n`;
