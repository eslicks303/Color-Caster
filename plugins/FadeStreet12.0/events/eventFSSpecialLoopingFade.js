...
const id = "GF_EVENT_FADE_STREET_SPECIAL_LOOPING_FADE";
const groups = ["Fade Street"];
const subGroups = {"Fade Street": "Colour Mode - Fade Only"};
const description = "Fades from one palette to another, then back again, with optional special colour effects. The effect loops seamlessly. The fade is non-linear, so the colours spend more time close to the ends of the gradient than in the middle.";
const name = "Looping Special Fade";
...
const autoLabel = (fetchArg) => {
	...
	return `Looping Special Fade from ${startpoint} to ${endpoint} (${frames} frames)`;
}
...
const compile = (input, helpers) => {
	...
	_addComment(`     `);
	_addComment(`                      Looping Special Fade`);
	_addComment(`            [GF_EVENT_FADE_STREET_SPECIAL_LOOPING_FADE]`);
	_addComment(``);
	...
	appendRaw(gbvm);
	_addComment(`~~~ End of Looping Special Fade block ~~~\n\n`);
};
