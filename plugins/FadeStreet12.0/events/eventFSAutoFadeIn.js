...
const id = "GF_EVENT_FADE_STREET_AUTO_FADE_IN";
...
const description = "Fade in from a solid colour to the selected palettes for this scene.";
const name = "Automagic Fade In";
...
const autoLabel = (fetchArg) => {
	...
	return `Automagic Fade In From ${startpoint} (${frames} frames)`;
}
...
const fields = [
	{
		...
		fields: [
			{
				label: "Fade from:",
				description: "Colour to fade in from.",
				width: "50%",
			},
			{
				defaultValue: "White",
...
const compile = (input, helpers) => {
	...
	let dmg_startpoint = white;
	switch (input.dmg_startpoint) {
		default:
		case "white":
	...
	const dmg_endpoint = default_dmg;
	...
	for (let i = 0 ; i < 64; ++i) { // 4 colours * 16 palettes
		...
		const OK_L_gradient = steps_arr.map((x) => OK_L1 + ((OK_L0 - OK_L1) / (steps - 1)) * x);
		const OK_a_gradient = steps_arr.map((x) => OK_a1 + ((OK_a0 - OK_a1) / (steps - 1)) * x);
		const OK_b_gradient = steps_arr.map((x) => OK_b1 + ((OK_b0 - OK_b1) / (steps - 1)) * x);
	...
	_addComment(`     `);
	_addComment(`                   Automagic Fade In`);
	_addComment(`          [GF_EVENT_FADE_STREET_AUTO_FADE_IN]`);
	_addComment(``);
	...
	_addComment(`~~~ End of Automagic Fade In block ~~~\n\n`);
};
